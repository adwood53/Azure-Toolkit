...
namespace LightBuzz.Kinect4Azure
{
    ...
    public class Stickman : MonoBehaviour
    {
        ...
        public void Load(Body body)
        {
            ...
            for (int i = 0; i < _points.Length; i++)
            {
                average += _points[i].transform.position;
            }
            average = average / _points.Length;


            if(playkit = GameObject.Find("Playkit"))
            {
                playkit.GetComponent<PointCloudScript>().skeletonPosition(average);
            }
